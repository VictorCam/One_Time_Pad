...
{
	FILE *check_len1 = fopen(argv[1],"r"); //original text dec
	FILE *check_len2 = fopen(argv[2],"r"); //key

	FILE *check_inside = fopen(argv[1],"r"); //check content dec

	FILE *origin_file = fopen(argv[1],"r"); //origin_file content dec
	...
	while (c1 != EOF) //check the length of the encryptionfile
	{
		c1 = fgetc(check_len1);
		count1++;
	}
...
//below the if accepts the following ascii values
//32 is space
//10 is 0
//-1 is \
//65-90 is A-Z
	...
	while (c4 != EOF)
	{
		c4 = fgetc(origin_file);
		c5 = fgetc(key_file);

		if(c4 == '\n' || c5 == '\n') //prevent from getting garbage
		{
			file1[y] = '-';  //add a - at first file
			file2[y] = '-'; //add a - at second file
			break;
		}
	...
	strcat(server_connect,"client_dec-"); //will be used to check if connection is valid
	char result[70000];
