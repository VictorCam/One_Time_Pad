...
//check if the client_enc is true else do not do anything
		if(strcmp("client_enc",strings[0]) == 0) { //check if the client is related to the server
	  ...
	  int char_int_F2;
		...
		while (string_length != y) //will loop until the stringlength is not equal to it's length amount
		{
			...
			char_int_F1 = c4; //converting origin file to int
			...
			{
				encrpyted = ((char_int_F1 - 65) + (char_int_F2 - 65)) % 27; //we use this algorithm to encrypt the file
		...
		send(establishedConnectionFD, connect_to, 70000, 0); //send back encrpytion
